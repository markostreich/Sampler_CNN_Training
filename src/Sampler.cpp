...
void mouseHandle(int event, int x, int y, int flags, void *param) {
  ...
  if (event == CV_EVENT_MOUSEMOVE && mouseIsDragging == true) {
    ...
    if (x_dist > y_dist)
      currentSecondPoint =
          Point(initialClickPoint.x + x_dist, initialClickPoint.y + x_dist);
    else
      currentSecondPoint =
          Point(initialClickPoint.x + y_dist, initialClickPoint.y + y_dist);
    currentSecondPoint = Point(x, y);
    objects.at(selectedRect - 1).rect =
        Rect2d(initialClickPoint, currentSecondPoint);
    objects.at(selectedRect - 1).active = true;
    objects.at(selectedRect - 1).selected = true;
    // if (debug){
    // 	Rect2d rect = objects.at(selectedRect-1).rect;
    // 	// cout << format("%f %f %f %f",
    // rect.tl().x,rect.tl().y,rect.br().x,rect.br().y) << endl;
    // }
...
void convertToYOLOLabels(int wFrame, int hFrame, double xtl, double ytl,
                         double xbr, double ybr, double &xObj, double &yObj,
                         double &wObj, double &hObj) {
...
void convertFromYOLOLabels(double xObj, double yObj, double wObj, double hObj,
                           int wFrame, int hFrame, double &xtl, double &ytl,
...
void draw_ObjectRects(Mat &frame) {
  for (vector<ObjectRect>::const_iterator it = objects.begin();
       it != objects.end(); ++it) {
    if ((*it).active) {
      if ((*it).selected) {
...
void showCommandlineUsage() {
  cerr << "\n"
       ...
       << "sam_sampler"
       << "\n"
       << "\t-m (or --mode) <recordonly|labelonly|labelvideo|recordandlabel|reviselabels|fpstest|listdevices|splitsets>\n"
       << "\t-s (or --source) <source: device number or video file>\n"
       << "\t-d (or --destination) <destination folder>"
       << "\n"
       << "\t-f (or --fps) <frames per second>"
       << "\n"
       << "\t-n (or --nextnumber) <image number to start labeling with in mode "
          "labelvideo>"
       << "\n\n"
       << "Defaults:"
       << "\n\n"
       << "sam_sampler"
       << " -m labelonly -s 0 -d Storage -f 60\n\n";
}
  ...
  for (int i = 1; i < argc; ++i) {
    if (string(argv[i]) == "-m" || string(argv[i]) == "--mode") {
      if (string(argv[i + 1]) == "recordonly") {
        ...
        mode = REVISE_LABELS;
      } else if (string(argv[i + 1]) == "splitsets") {
        mode = SPLIT_SETS;
      } else {
        showCommandlineUsage();
        return 1;
...
std::string get_file_contents(const string filename) {
  ...
  if (in) {
    string result = (std::string((std::istreambuf_iterator<char>(in)),
...
void safeYOLOLabelsRevision(string pathYOLOLabel, Mat frame) {
  ...
  memcpy(dist_files_path, pathYOLOLabel.c_str(), pathYOLOLabel.size());
  ...
  for (unsigned int i = 0; i < objects.size(); i++) {
    if (objects.at(i).active) {
      ...
      convertToYOLOLabels(frame.cols, frame.rows, tl.x, tl.y, br.x, br.y, xObj,
                          yObj, wObj, hObj);
      opendistFile << objects.at(i).classification << objects.at(i).direction
                   << " " << xObj << " " << yObj << " " << wObj << " " << hObj
                   << endl;
    }
  }
...
void reviseLabels(const string p) {
  ...
  int cntFiles =
      std::count_if(directory_iterator(pathRGB), directory_iterator(),
  ...
  while (true) {
    ...
    string yoloString = get_file_contents(p + "/YOLO_Labels/" +
                                          get_yolo_file(files[fileIndex]));
    ...
    while (i <= yoloValues.size() - 1 && yoloValues.size() >= 5) {
      ...
      convertFromYOLOLabels(xObj, yObj, wObj, hObj, image.cols, image.rows, xtl,
                            ytl, xbr, ybr);
    ...
    while (!next) {
      ...
      putText(image, get_yolo_file(files[fileIndex]), Point(10, 470), 1, 1,
              Scalar(0, 255, 255), 1);
      ...
      if (key == '+' && fileIndex + 1 < cntFiles) {
        ...
        if (all_classes_set())
          safeYOLOLabelsRevision(
              p + "/YOLO_Labels/" + get_yolo_file(files[fileIndex]), image);
        else {
          const string str = "Failure: Unset classification";
...
/**
 * Creates a validation or a test set depending on mode.
 * @param p Directory with labels and samples.
 */
void createSubSets(string p) {
	if (!exists(p)){
		printf("No such directory!\n");
		return;
	}
  ...
  const path pathValidation(p + "/ValidationSet");
  const path pathTest(p + "/TestSet");
  createSuperFolder(pathValidation.string());
  createSubFolder(pathValidation.string() + "/RGB",
                  pathValidation.string() + "/YOLO_Labels");
  createSuperFolder(pathTest.string());
  createSubFolder(pathTest.string() + "/RGB",
                  pathTest.string() + "/YOLO_Labels");
  /*Andrew Ng recomments a ratio of 60-20-20 training-validation-test set*/
  for (int fileIndex = 0; fileIndex < cntFiles * 0.4; fileIndex++) {
    if (fileIndex < cntFiles * 0.2) {
			rename(files[fileIndex], pathValidation.string() + "/RGB/" + splitString(files[fileIndex], '/').back());
			rename(p + "/YOLO_Labels/" + get_yolo_file(files[fileIndex]), pathValidation.string() + "/YOLO_Labels/" + get_yolo_file(files[fileIndex]));
    } else {
			rename(files[fileIndex], pathTest.string() + "/RGB/" + splitString(files[fileIndex], '/').back());
			rename(p + "/YOLO_Labels/" + get_yolo_file(files[fileIndex]), pathTest.string() + "/YOLO_Labels/" + get_yolo_file(files[fileIndex]));
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (mode == FPS_TEST) {
    ...
    return 0;
  } else if (mode == SPLIT_SETS) {
		createSubSets(destination);
		return 0;
	}
  ...
  while (1) {
    while (!startTracking) {
      ...
      if (countFrames < 3)
        countFrames++;
      if ((mode == LABEL_VIDEO && countFrames == 3) ||
          (mode == RECORD_AND_LABEL && countFrames == 3)) {
        doCapturing = false;
      }
      key = waitKey(1);
      if (key == 32) {
    ...
    while (32 != key) {
      ...
      if (safeToFiles) {
        string imageFileName = format("%simage", path_RGB) + "-" + currentDate +
                               "-" + format("%04d", image_counter) + ".JPEG";
      ...
      if (safeToFiles) {
        if (YOLOLabels) {
          // store labels in distinct files
          const string distFilesPath = format("%simage", path_Label) + "-" +
                                       currentDate + "-" +
                                       format("%04d", image_counter) + ".txt";
          char *dist_files_path = new char[distFilesPath.size() + 1];
          dist_files_path[distFilesPath.size()] = 0;
          memcpy(dist_files_path, distFilesPath.c_str(), distFilesPath.size());
          opendistFile.open(dist_files_path);
        }
      }
      for (unsigned int i = 0; i < tracker->objects.size(); i++) {
        ...
        if (safeToFiles) {
          // <filename> <classname> <middle.x> <middle.y> <object width> <object
          // height> <image width> <image height>
          string imageFileName = "image-" + currentDate + "-" +
                                 format("%04d", image_counter) + ".JPEG";
          openFile << imageFileName << " " << objects.at(j).classification
                   << objects.at(j).direction << " " << (int)((br.x - tl.x) / 2)
                   << " " << (int)((br.y - tl.y) / 2) << " " << br.x - tl.x
                   << " " << br.y - tl.y << " " << frame.cols << " "
                   << frame.rows << endl;

          if (YOLOLabels) {
            // store labels in distinct files for YOLO
            double xObj, yObj, wObj, hObj;
            convertToYOLOLabels(frame.cols, frame.rows, tl.x, tl.y, br.x, br.y,
                                xObj, yObj, wObj, hObj);
            // <filename> <classname> <relative middle.x> <relative middle.y>
            // <relative width> <relative height>
            opendistFile << objects.at(j).classification
                         << objects.at(j).direction << " " << xObj << " "
                         << yObj << " " << wObj << " " << hObj << endl;
          }
        }
        rectangle(frame, objects.at(j).rect, objects.at(j).color, rect_linesize,
                  1);
      }
      ...
      const string fpsStr =
          format("%.1f", fps); // boost::lexical_cast<string>(round(fps));
      putText(frame, fpsStr + " FPS", Point(490, 30), 1, 2, Scalar(0, 255, 255),
              2);
      imshow(windowName, frame);
